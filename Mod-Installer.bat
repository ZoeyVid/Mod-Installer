...
if not exist "%ProgramFiles(x86)%\Minecraft Launcher\MinecraftLauncher.exe" (
	CLS
	echo Der Minecraft Launcher konnte nicht am Ã¼blichen Pfad gefunden werden!
...
if not exist "%appdata%\.minecraft" (
    CLS
    echo .minecraft Ordner nicht am Ã¼blichen Pfad oder nicht vorhanden. 
...
CLS
	...
	ECHO i) Profil Update            - Lade ein Profil um es zu aktualisieren und danach zu Ã¼berschreiben!
	ECHO.
...
echo Im nâ€žchsten Schritt ”ffnet sich automatisch ein Fenster klicke dort nur auf "OK"!
Pause
java -jar forge.jar
del /S /Q forge.jar
...
echo Im nâ€žchsten Schritt ”ffnet sich automatisch ein Fenster klicke dort nur auf "Install"!
Pause
...
ECHO.
ECHO a) KÃ¼rbis Sicht entfernen                   - KÃ¼rbis.zip
...
ECHO e) Textures der neuen Versionen fÃ¼r die 1.8 - https://lksr.de/18new
...
ECHO 3. 1.8
CHOICE /C 123 /M "Auswahl: "
IF ERRORLEVEL 3 curl -L -o KÃ¼rbis-1.8.zip  https://dl.san0j.de/mods/rp/Kuerbis-1.8.zip
IF ERRORLEVEL 2 curl -L -o KÃ¼rbis-1.16.zip  https://dl.san0j.de/mods/rp/Kuerbis-1.17.zip
IF ERRORLEVEL 1 curl -L -o KÃ¼rbis-1.17.zip  https://dl.san0j.de/mods/rp/Kuerbis-1.16.zip
GOTO rpif
...
cd "%appdata%\.minecraft\"
CLS
ECHO Stelle sicher, dass du im Minecraft Launcher unten Links den richtigen Modloader und MC-Version auswâ€žhlst!
ECHO Es werden nur Mods in den Profilen gespeichert und geladen!
ECHO In dem Backup auch Configs!
ECHO.
ECHO 1. Profil erstellen - Limit 8 - Vorhandene werden Ã¼berschrieben
ECHO 2. Profil laden
ECHO 3. Profil l”schen
ECHO 4. Backup laden - Automatisch bei Mod-Installation und MC-Start erstellt - Mods werden Ã¼berschrieben
ECHO.
...
CLS
ECHO.
ECHO Welches Profil m”chtest du aktualisieren (Das Profil wird geladen damit alle Config Ã¼bernommen werden)?
